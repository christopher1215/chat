...
- (NSArray *)emailAddresses {
    ...
    //get email child node (USERID)
    NSXMLElement *usrIdElement = [emailElement elementForName:@"USERID"];
    //get email
    NSString *email = [usrIdElement stringValue];
    //only get USERID
    if (email.length > 0) {
        return @[email];
    }
...
- (void)setEmailAddresses:(NSArray *)emails {
    /**
     ...
     <INTERNET />
     <PREF />
     <USERID>lisi@it.cn</USERID>
     </EMAIL>
     */
    
    //assary email must be only one
    NSAssert(emails.count == 1, @"email must be only one");
    //get email node
    NSXMLElement *emailElement = [self elementForName:@"EMAIL"];
    //remove USERID node
    [emailElement removeElementForName:@"USERID"];
    //create a new USERID node
    NSXMLElement *usrIdElement = [NSXMLElement elementWithName:@"USERID" stringValue:emails[0]];
    //add USERID node
    [emailElement addChild:usrIdElement];
